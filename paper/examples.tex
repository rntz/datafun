...
For purposes of these examples, we use a simple Haskell-like syntax
for top-level type and function definitions. We also permit ourselves
infix notation, \ms{let}-binding, $n$-ary tuples, $n$-ary sum types
with named constructors, and a restricted form of pattern-matching
(including non-linear patterns), and additional syntax sugar given in
Figures~\ref{fig:sugar} and~\ref{fig:pattern-expand}. These figures
also show how to expand the sugar (including pattern matching) into
our core language.

All of these conveniences are supported (with slightly different
concrete syntax) in our implementation. Our implementation also
supports fully general pattern matching, as in ML and Haskell, but we
left this out of the paper because we wanted to keep the desugaring as
simple as possible and did not need it for our examples.
