...
top-level type and function definitions. We also permit ourselves let-binding,
$n$-ary product types, \todo{labelled sum-types} and
pattern-matching\todo{(CITE)}, and additional syntax sugar given in Figure
\ref{fig:sugar}. All of these conveniences are supported (with slightly
...
Although, \todo{for the sake of simplicity(?)}, Datafun as presented does not
have polymorphism
...
\todo{Is this really a great example? It's basically a polymorphism example and
...
\begin{figure}
  \begin{center}
    \begin{tabular}{cl}
      ...
      $\FM{A}{P}$ & Poset of finite maps from $A$ to $P$
      \vspace{0.5em}\\
      \multicolumn{2}{c}{\textbf{Usl notation}}\\
      $L \lol M$ & Pointwise poset on usl-morphisms $\ms{Usl}(L,M)$\\
      $\Sups{P}$ & Free usl on an underlying poset $P$\\
      $\FS{A}$ & Free usl on the set $A$; same as $\Sups{(\Disc{A})}$\\
    \end{tabular}
  \end{center}
...

The semantics for Datafun is given in terms of three categories (\ms{Set},
\ms{Poset}, and \ms{Usl}) and two adjunctions between them (see Figure
\ref{fig:sem-cats}). We use nonstandard notation to avoid confusion between
operations on sets and posets (see Figure \ref{fig:sem-notation}). We take less
care to distinguish posets and usls, since while a set can be partially ordered
in many ways, a poset simply either \emph{is} or \emph{is not} an usl.

\subsection{The category \ms{Usl}}

The category \ms{Usl} is the category of join-semilattices with least elements
(``\emph{usls}''). Directly, an usl is a poset $P$ in which every finite subset
$X \subseteq_{\ms{fin}} |P|$ has a least upper bound $\bigvee X$ in $P$. We
define $\unit$ and $\vee$ with respect to an arbitrary usl as follows:
\begin{eqnarray*}
  \unit &=& \textstyle\bigvee \emptyset\\
  a \vee b &=& \textstyle\bigvee \{a,b\}
\end{eqnarray*}

An \ms{Usl}-morphism $f \in \ms{Usl}(A,B)$ is a function from $A$ to $B$
...
\ms{Usl} is a subcategory of \ms{Poset}; every \ms{Usl}-morphism is monotone,
since $a \le b \iff a \vee b = b$, and so given $a \le b$ we have $f(a) \vee
f(b) = f(a \vee b) = f(b)$, thus $f(a) \le f(b)$ for any \ms{Usl}-morphism $f$.

\subsection{Denotation of Datafun types}
Datafun types denote posets (see figure \ref{fig:semtypes}). Datafun's usl types
denote posets which are also usls --- and moreover, are usls where the
least-upper-bound operator $\bigvee$ is
\emph{computable}.\todo{\footnote{\todo{should this have a proof?}}}
...
%% \ms{Set} is isomorphic to a subcategory of \ms{Poset}, since a set $A$ may be
%% identified with the discrete poset on $A$ (in which $x \le x$ for every $x \in
%% A$, but otherwise no inequalities hold).
