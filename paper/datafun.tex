...
Datafun can be thought of as a simply-typed $\lambda$-calculus extended in four
major ways:
\begin{enumerate}
\item We add a type of \emph{monotone functions}, $A \mto B$. Consequently
  ...
  monotone variables. This distinction is left latent in the syntax itself,
  \todo{TODO: why?}.
...
\item We add a type of finite sets $\Set{A}$. We use finite sets to represent
  ...
  \todo{TODO: make clearer bit about what we use finite sets for}

  Sets are ordered by inclusion: $a \le b : \Set{A}$ iff $a \subseteq b$. Maps
  are ordered both by inclusion and pointwise.

\item We add a \emph{monotone fixed point} operator over \emph{finite-set-typed}
  expressions. This allows us to express Datalog-style recursion.

\item Generalizing the \emph{union} operator $\cup$ and the empty set
  $\emptyset$, we identify a \todo{syntactic} subset of types as \emph{usl
  types}, which support a \emph{least-upper bound} operation $\vee$ and have a
  \emph{least element} $\unit$.

  \emph{usl} stands for ``unitary semilattice'', and signifies that the partial
  orderings on these types form join-semilattices with a least element (unit).
  We use the metavariables $L,M$ for usl types.

  Usl types provide a natural comprehension-style eliminator for sets: given $e
  : \Set{A}$ we may write $\letin{x}{e}{e'}$ for the least-upper-bound, over all
  elements $x \in e$, of $e'$, as long as $e' : L$ is of usl type.
  ...
  \todo{Perhaps explain monotone fixed point first?}
