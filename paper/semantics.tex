...
\paragraph{}
For the case of bounded fixed-points $(\fixle{\m{x}}{e_1}{e_2})$, we need a
further lemma:

\paragraph{Lemma:}
The denotation $\den{L}$ of any semilattice equality type $\eq{L}$ is
\emph{all-finite-height}: that is, for any $x \in \den{\eq{L}}$, the height of
$\below{x}{\den{\eq{L}}}$ is finite.

\paragraph{Proof:} By induction over semilattice equality types $\eq{L}$. \qed
