...
To complete our semantics, we require a few simple lemmas about the denotations
of Datafun types. Firstly, we need to know that our semilattice types are
semilattices, and that our finite types are finite:
...
\paragraph{}
Secondly, to show that bounded fixed-points $(\fixle{\m{x}}{e_\top}{e})$
...
Any monotone map $f : A \to A$ on a poset $A$ of finite height with a least
...
x$. Thus $f^n(\unit)$ is the least fixed point of $f$. \qed

\paragraph{}
We notate the least-fixed point of a monotone function $f$ on a semilattice $L$
of finite height as $\lfpin{L}{f}$.
