...
\begin{enumerate}
\item Assume we subscript all semilattice operations ($\unit$, $\vee$,
  $\bigvee$, and $\ms{fix}$) with their type.
...
%% FIXME
In our rules for $e \step e'$ we make use of a decidable ordering test on
values, $v \ale u : \eq{A}$, and a corresponding equality test $v \aeq u :
\eq{A}$.
