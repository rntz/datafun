...
As a matter of notation, $a,b,c$ range over closed expressions; $e,f,g$ over
open expressions; $\gamma, \sigma$ over substitutions containing only closed
expressions; and $\ms{Ctx}(\GG)$ is the set of all substitutions of closed
expressions for the variables in $\GG$.

Our logical relations mimic our semantics, with a slight twist. We interpret
types as sets of closed terms, but instead of equipping these sets with partial
orders, we equip them with preorders, since distinct closed expressions may have
equal denotations. To accomplish this, we inductively define relations $\lr{A}{a
  \ale b}$ for each type $A$, then show how to understand these relations as
preordered sets.
...
aRb$.\footnote{Interestingly, such an $R$ may also be viewed as a partial
...
\end{theorem}
\begin{proof}
  By induction on $\J{\GD}{\GG}{e}{A}$; full proof available in our github
  repository.
\end{proof}
