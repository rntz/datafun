...
\begin{corollary}\label{cor:fineq-stops}
  ...
  sequence $\stepfn(x.e)^i(a)$ has a stopping point, i.e. there is some $j$ such
  that $\forall k \ge j.\ \lr{\fineq{L}}{\stepfn(x.e)^j(a) \equiv
    \stepfn(x.e)^{k}(a)}$.
\end{corollary}
\begin{proof}
...
\begin{lemma}\label{lem:iter-onestep}
  ...
  \begin{cases}
    \iter{\fineq{L}}{\stepfn(x.e)(v)}{x}{e}
    & \text{if}~v \not\equiv \stepfn(x.e)(v)~\text{in}~\Val{\fineq{L}}\\
    v & \text{otherwise}
  \end{cases}
  \]
\end{lemma}
\begin{proof}
  By theorem \ref{thm:agree-ineq}, the rules for $\step$, and the definition of
  $\stepfn(x.e)(v)$.
\end{proof}
...
\begin{proof}
  ...
  For any $j \le i$ we have $\stepfn(x.e)^j(v) \not\equiv \stepfn(x.e)^{j+1}(v)$
  and so by induction, applying lemma \ref{lem:iter-onestep}, we have:
  \[
  \iter{\fineq{L}}{v}{x}{e} \steps \iter{\fineq{L}}{\stepfn(x.e)^i(v)}{x}{e}
  \]
  ...
  \begin{array}{rlll}
    \iter{\fineq{L}}{v}{x}{e}
    &\steps& \iter{\fineq{L}}{\stepfn(x.e)^i(v)}{x}{e}\\
    &\steps& \stepfn(x.e)^i(v)
...
\begin{proof}
  ...
  \item[Case] $\infer[\ms{fix}]{\J{\GD}{\GG}{\fix{\m{x}} e}{\fineq{L}}}{
    ...
    \stepfn(x.\gamma_j(e))^i(\evalfn(\unit_{\fineq{L}}))$ where $i$ is the
    sequence index and $j \in \{1,2\}$. By corollary \ref{cor:fineq-stops} these
    sequences each have stopping points; call them $i_1, i_2$ respectively. By
    lemma \ref{lem:lr-unit} $\unit_{\fineq{L}} \ale \unit_{\fineq{L}}$ and by
    theorem \ref{thm:stepfn-monotone}
    $\lrcx{y\of\fineq{L}}{\fineq{L}}{\stepfn(x.\gamma_1(e))(y) \ale
      \stepfn(x.\gamma_2(e))(y)}$, so by induction $S_i^1 \ale S_i^2$. Thus by
    theorem \ref{thm:stopping-points-le}, $S^1_{i_1} \ale S^2_{i_2}$.

    Now, observe that
    \[\begin{array}{rlll}
      \tfix{\fineq{L}}{x}{\gamma_j(e)}
      &\steps& \iter{\fineq{L}}{\unit_{\fineq{L}}}{x}{\gamma_j(e)}\\
      &\steps& \iter{\fineq{L}}{\evalfn(\unit_{\fineq{L}})}{x}{\gamma_j(e)}\\
      &\equiv& S_i^j & \text{by theorem \ref{thm:iter-stops}}
    \end{array}\]
