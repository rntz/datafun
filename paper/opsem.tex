...

\todo{NOTE: It is \emph{not} the case that ``partial preorders'' (sets equipped
  with a partially-reflexive transitive relation) are isomorphic to PERs
  equipped with partial orders respecting the equivalence relation,
  \textbf{because} partial reflexivity (as I've defined it) only means $a \ale b
  \implies a \ale a$, and not also $a \ale b \implies b \ale b$.

  In other words, in a ``partial preorder'', an $a$ that is ``in'' the preorder
  ($a \ale a$) can be below a $b$ that is not ``in'' the preorder ($b \not\ale
  b$).

  Now, I believe all the partial preorders I consider here have this additional
  property, and therefore (I think) correspond to partially-ordered PERs, but
  none of the proofs depend on this fact.}
...
%% \newcommand{\ordereq}{\simeq}
%% \begin{definition}
%%   Two preorders $P, Q$ are \emph{order-equivalent}, written $P \ordereq Q$, iff
%%   there exist functions $f : P \to Q$, $g : Q \to P$ such that
%%   \begin{eqnarray*}
%%     x \ale y &\implies& f(x) \ale f(y)\\
%%     x \ale y &\implies& g(x) \ale g(y)\\
%%     x &\equiv& f(g(x))\\
%%     x &\equiv& g(f(x))\\
%%   \end{eqnarray*}

%%   Note that two posets are order equivalent iff they are order isomorphic in the
%%   usual sense.

%%   We show that $f(x) \ale f(y) \implies x \le y$:
%%   \begin{eqnarray*}
%%     f(x) \ale f(y) &\implies& g(f(x)) \ale g(f(y))\\
%%     &\implies& x \ale y
%%   \end{eqnarray*}

%%   \todo{wait, isn't being order-equivalent just saying their equivalence posets
...
\begin{proof}
  ...
  %% \item[Case $\N$] $\down{n} = \{0..n\}$
  %% \item[Case $\str$] Since $\str$ has the discrete ordering, $\down{v} = \{v\}$.
