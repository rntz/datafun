...
by picking a representative of each equivalence class), this implies that the
height of $P$ and the height of $\eqposet{P}$ are equal for any preorder $P$.
...
\begin{proof} By induction on $\fineq{A}$:
  ...
  \item[Case $\Set{\fineq{A}}$] All values $\lr{\Set{\fineq{A}}}{v}$ have the
    ...
    Since any chain in a preorder $P$ may be extended to a chain in the
    partially ordered set of equivalence-classes of $P$ (taking $[x] = [y]$ iff
    $x \le y \wedge y \le x$) \TODO
