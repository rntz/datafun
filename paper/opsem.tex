...
\begin{definition}
  A \emph{stopping point} of a sequence in a preorder $P$ is an $a_i$ such that
  \(\forall j \ge i.\ a_i \equiv a_j\).
\end{definition}
...
\begin{lemma}\label{lem:iter-stops}
  Given $\lr{\fineq{L}}{v}$ and $\lrcx{x\of\fineq{L}}{\fineq{L}}{e}$, for $i$ a
  stopping point of $\stepfn(x.e)^i(v)$ (guaranteed to exist by theorem
  \ref{thm:fineq-stops}), then
...
\begin{proof}
  ...
  %%
  %% \begin{description}
  %% \item[Case $0$:] From $\stepfn(x.e)^0(v) \equiv \stepfn(x.e)^1(v)$ we have $v
  %%   \equiv \evalfn(\sub{v/x}e)$, thus by theorem \ref{thm:agree-ineq} $v \equiv
  %%   \evalfn(\sub{v/x}e) : \fineq{L}$. Thus:
  %%   \[\begin{array}{rlll}
  %%     \iter{\fineq{L}}{v}{x}{e}
  %%     &\step& \iterstep{\fineq{L}}{v}{\sub{v/x}e}{x}{e}\\
  %%     &\steps& \iterstep{\fineq{L}}{v}{\evalfn(\sub{v/x}e)}{x}{e}\\
  %%     &\step& v & \text{because}~v \equiv \evalfn(\sub{v/x}e) : \fineq{L}\\
  %%     &\equiv& \stepfn(x.e)^0(v)
  %%   \end{array}\]
  %% \item[Case $i+1$:] We know $\stepfn(x.e)^0(v) \not\equiv \stepfn(x.e)^1(v)$.
  %%   Therefore:
  %%   \[\begin{array}{rlll}
  %%     \iter{\fineq{L}}{v}{x}{e}
  %%     &\step& \iterstep{\fineq{L}}{v}{\sub{v/x}e}{x}{e}\\
  %%     &\step*& \iterstep{\fineq{L}}{v}{\evalfn(\sub{v/x}e)}{x}{e}\\
  %%     &\step& \iter{\fineq{L}}{\evalfn(\sub{v/x}e)}{x}{e}
  %%   \end{array}\]
...
\begin{proof}
  ...
  \item[Case] $\infer[\ms{fix}]{\J{\GD}{\GG}{\fix{\m{x}} e}{\fineq{L}}}{
    ...
    Consider the sequences $S^j_i = \stepfn(x.\gamma_j(e))^i(\unit_{\fineq{L}})$
    where $i$ is the sequence index and $j \in \{1,2\}$. By corollary
    \ref{cor:fineq-stops} these sequences each have stopping points; call them
    $i_1, i_2$ respectively.

    \todo{show $\lr{\fineq{L}}{S^1_{i_1} \ale S^2_{i_2}}$; need a simple lemma
      here}

    \todo{show $\tfix{\fineq{L}}{x}{\gamma_j(e)} \equiv S^j_{i_1}$ - this is
      easy, step the \ms{fix} and then apply lemma \ref{lem:iter-stops}.}
