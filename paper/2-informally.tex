...
\begin{enumerate}
\item We add a type of finite sets, $\Set{A}$. We also include finite
  maps\footnote{By ``finite maps'', we mean not ``total maps from a finite
    domain'', but rather ``partial maps with finitely many present keys'', also
    called \emph{dictionaries}.} $\Map{A}{B}$, as they are useful for some
  examples.
