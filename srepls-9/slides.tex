...
\begin{frame}{1. Exploiting \textbf{Poset}: \path}
  $\pathof{A}$ identifies weakly connected elements in $A$:
...
\begin{frame}{2. Bottom-up mode inference}
  \LARGE
  \begin{mathpar}
...
\begin{frame}{2. Bottom-up mode inference: composition}
  \LARGE
  \[
  ...
  \]
\end{frame}
%% TODO: frame about composition of tones.

\begin{frame}{2. Bottom-up mode inference: composition}
  Composition forms a monoid with $\id$ as identity:
  \begin{mathpar}
    \begin{array}{cr|cccc}
...
%% \begin{frame}{3. Modal subtyping: \iso-elimination}
%%   \huge
%%   \[ \fnof{x} x : \isof{\R} \to \R \pause\iff \isof{\R} \subtype \R \]
%% \end{frame}

\begin{frame}{3. Modal subtyping: \iso-introduction?}
  \LARGE
  \[
  \infer[\rulename subtyping]
...
\begin{frame}{3. Modal subtyping!}
  ...
  \pause{} Then subtyping is modal composition:
  \LARGE
...
\begin{frame}{3. Modal subtyping: \iso-introduction!}
  \LARGE\vspace{-1em}
  \begin{mathpar}
  \infer{\m{T} A \subtype B}{\m{\iso T} A \subtype \isof{B}}
  \vspace{1em}\\\pause
  \infer[\rulename subtyping as \iso{}-introduction]
        {x : \m{\hil T} A \vdash M : B
