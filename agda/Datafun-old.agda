...
push : {Ω Ω′ : Env} {a : Type} (tone : Tone) → (Ω ⊢* Ω′)
     → (a # tone ∷ Ω ⊢* a # tone ∷ Ω′)
push mono s = (mono , var car) · (↑ • s)
push disc s = (disc , var car) · (↑ • s)

subst : {Ω Ω′ : Env}{a : Type} → Ω ⊢* Ω′ → Ω′ ⊢ a → Ω ⊢ a
subst id M = M
subst (s • t) M = subst s (subst t M)
subst ↑ (var x) = var (cdr x)
subst ((disc , N) · s) (var car) = subst (unwipe _) N
subst ((mono , N) · s) (var car) = N
subst (_ · s) (var (cdr x)) = subst s (var x)
-- subst s (fix x M) = fix x (subst ((mono , var car) · (↑ • s)) M)
subst s (fix x M) = fix x (subst ((mono , var car) · (↑ • s)) M)
subst s (ε p) = ε p
