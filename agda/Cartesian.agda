...
-- Technically, none of this needs Cat. It just needs SetRel!
module _ {i}{j} (C : Cat i j) where
  ...
  record Products (_⊗_ : Obj C -> Obj C -> Obj C) : Set (i ⊔ j) where
    constructor Products:
    field π₁ : ∀{A B} -> A ⊗ B ⇨ A
    field π₂ : ∀{A B} -> A ⊗ B ⇨ B
    field ⟨_,_⟩ : ∀{A B C} -> A ⇨ B -> A ⇨ C -> A ⇨ B ⊗ C

  record Sums (_⊕_ : Obj C -> Obj C -> Obj C) : Set (i ⊔ j) where
    constructor Sums:
    field in₁ : ∀{A B} -> A ⇨ A ⊕ B
    field in₂ : ∀{A B} -> B ⇨ A ⊕ B
  ...
  record Closed (_⊗_ : (a b : Obj C) -> Obj C)
                (hom : (a b : Obj C) -> Obj C)
                : Set (i ⊔ j) where
    constructor Closed:
