...
change-SL P S = Change: {{P}} P (λ {a} {b} a≤b da → a ∨ da ≈ b) init
-- (λ da a b → a ∨ da ≈ b)
...
module _ (A B : Change) where
  ...
  change× = Change: {{𝑶 A ∧ 𝑶 B}} (𝑫 A ∧ 𝑫 B)
                    (λ { (p , q) (da , db) → Path A p da × Path B q db })
                    (dummy A , dummy B)

  data Path+ : {a b : 𝑶 A .Obj ⊎ 𝑶 B .Obj}
             -> .(p : (rel+ (𝑶 A .Hom) (𝑶 B .Hom) a b)) (q : 𝑫 A .Obj ⊎ 𝑫 B .Obj)
             -> Set where
    rel₁ : ∀{a b} .{a≤b : a ≤ b} {p} -> Path A a≤b p -> Path+ (rel₁ a≤b) (inj₁ p)
    rel₂ : ∀{a b} .{a≤b : a ≤ b} {p} -> Path B a≤b p -> Path+ (rel₂ a≤b) (inj₂ p)
