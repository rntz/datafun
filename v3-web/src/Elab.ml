...
let underBinder k = Infer.locally (fun cx -> {cx with depth = cx.depth + 1}) k

let withVarAs (tone:tone) (var:var) (tp:tp) (k: 'a infer): 'a infer =
  fun cx ->
  let cx = {cx with depth = cx.depth + 1;
                    vars = Vars.add var (cx.depth, tp) cx.vars} in
...
type expInfer = (tp * Backend.exp) infer
...
let elabExp (e: alg expF): alg = fun expect ->
  let open Infer in
  getContext >>= fun cx ->
  ...
  let checkOnly (f: tp -> Backend.exp infer): expInfer = match expect with
  ...
  | `Fix (pat, body) ->
     checkOnly **> fun tp ->
     let mkFix x = `Fix (Backend.zeroExp (semilattice tp), patName pat, x) in
     (* TODO: check pattern is irrefutable *)
     map mkFix **> underBinder begin
