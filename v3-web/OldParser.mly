...
(* helpers for patexpr rules. *)
let map f g (x,y) = (Option.map f x, Option.map g y)
let pureExpr x = (None, Some x)
let mapExpr f (_,y) = (None, Option.map f y)
(* let mapPat f (x,_) = (Option.map f x, None) *)
(* TODO: function that tests if (None,None) and parse errors if so *)
let annot x = map id (fun e -> E(getpos(),e)) x
(* let annot (x,e) = (x, Option.map (fun e -> E(getpos(),e)) e) *)

module OptionList = Lists.Seq(Option)
%}
...
%type <Ast.pat option * Ast.loc Ast.exp option> patexp
%type <Ast.pat option * Ast.loc Ast.exp option> test_patexp
