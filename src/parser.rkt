...
(provide parse-port parse-string parse-file parser-for
         datafun-lex
         decls-parse expr-parse type-parse pat-parse
         ;; FIXME XXX
         ;; (all-from-out "datafun-syntax.rkt")
         )

(require "util.rkt" "debug.rkt" "lex.rkt")
(require (prefix-in : "datafun-syntax.rkt"))
...
;; parser-tools/yacc, and produces source info in the format expected by
;; datum->syntax.
(define (make-source-info start-pos end-pos)
  (match-define (position start line column) start-pos)
  (define span (- (position-offset end-pos) start))
  (vector (current-source-name) line column start span))

;; Attaches source information to a value (which is converted to a syntax object
;; if it wasn't already).
(define (with-source-info stx source-info)
  (if (syntax? stx)
      (datum->syntax stx (syntax-e stx) source-info)
      (datum->syntax #f stx source-info)))
...
(define-syntax-parser current-source-info
  ...
  [(_ ctx) (with-syntax ([start-pos (datum->syntax #'ctx '$1-start-pos)]
             ...
             #'(make-source-info start-pos end-pos))])

;; Annotates a value with the current source info.
(define-syntax-parser Q
  [(_ e ctx) #'(with-source-info e (current-source-info ctx))]
  [(_ e)     #'(Q e e)]
  [self:id   #'(lambda (e) (Q e self))])
...
(define (expr-parse . _) TODO)
(define (decls-parse . _) TODO)
(define (pat-parse . _) TODO)

;; this still has some shift-reduce conflicts, but I don't care.
(match-define type-parse ;; (list decls-parse type-parse pat-parse expr-parse)
  (parser
   (start type)
   ...
   ;; production names ending in a dash, eg "t-fun-", indicate that they produce
   ;; (potentially) non-source-position-annotated results.
...

    ...
    (tone? (() '()) ((tone) (list $1)))
    (tone ((CONST) (Q #':any)) ((MONO) (Q #':mono)) ((ANTI) (Q #':anti)))
...

    ...
    (t-fun
     ((t-arg)           $1)
     ((t-arg ->  t-fun) (Q #`(:Fun :any  #,$1 #,$3)))
     ((t-arg ~>  t-fun) (Q #`(:Fun :mono #,$1 #,$3)))
     ((t-arg ->+ t-fun) (Q #`(:Fun :mono #,$1 #,$3)))
     ((t-arg ->- t-fun) (Q #`(:Fun :anti #,$1 #,$3))))
    (t-arg
     ((t-product) (match $1
                    [(list x) x]
                    [xs (Q #`(:Tuple #,@xs))])))
    ...
    (t-ctor
     ((Id)              (Q #`(#,$1)))
     ((Id LP t-list RP) (Q #`(#,$1 #,@$3))))

    (t-atom
     ((LP t-paren RP)                   $2)
     ((id)                              (Q #`(:Name #,$1)))
     ((LCURLY t-paren RCURLY)           (Q #`(:Set #,$2)))
     ((LCURLY type : t-paren RCURLY)    (Q #`(:Map #,$2 #,$4))))
...

    ...
    (pat-
     ...
     ((pat & pat)       #`(:and #,$1 #,$3))
     ((LP pat- RP)      $2)
     ((LP p-list* RP)   #`(:tuple #,@$2))
     ((Id)              #`(:tag #,$1))
     ((Id LP p-list RP) #`(:tag #,$1 #,@$3)))
    (p-list* (() '())
...

    ...
    ;;  ((LAMBDA names => expr)        (e-lam* (current-source-info) $2 $4))
    ...
    ;;  ((e-app)                $1))
    ;; (e-oper
    ;;  ((oper) (Q (if (prim? $1) (e-prim $1) (e-var $1)))))
    ;; (e-app
    ;;  ((e-app e-atom)   (Q (e-app $1 $2)))
    ;;  ((e-atom)         $1))
    ;; (e-atom ((e-atom-) (Q $1)))
    ;; (e-atom-
    ...
    ;; (case  ((BAR pat => expr) (Q (case-branch $2 $4))))
