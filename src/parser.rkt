...
(provide source-info get-source-info parse-port parse-string parse-file)
...
(define (annotate-with! obj start-pos end-pos)
  (when (source-info)
    (hash-set! (source-info) obj (list start-pos end-pos)))
  obj)

;; magic variable-capturing macro
(define-syntax-parser annotate!
  [(_ e ctx) (with-syntax ([start-pos (datum->syntax #'ctx '$1-start-pos)]
               ...
               #'(annotate-with! e start-pos end-pos))]
  [(_ e) #'(annotate! e e)]
...
(match-define (list decls-parse decl-parse type-parse pat-parse expr-parse)
  (parser
   ...
   (grammar
    ...
    (t-factor
     ((t-atom)                      $1)
     ((SET t-atom)                  (annotate! `(set ,$2)))
     ((MAP t-atom t-atom)           (annotate! `(map ,$2 ,$3))))
    ...
    (t-atom
     ((id)              $1)
     ((LP t-big RP)     $2)
     ((LCURLY t-big RCURLY)         (annotate! `(set ,$2)))
     ((LCURLY type : t-big RCURLY)  (annotate! `(map ,$2 ,$4))))
    (t-big                              ; a type inside parentheses or similar
     (()    '(*))
     ((type) $1)
     ((type COMMA t-list) (annotate! `(* ,$1 ,@$3))))
    (t-list
     (()                   '())
     ((type)               `(,$1))
    ...
    (pat
     ((name)            $1)
     ((lit)             $1)
     ((_)               '_)             ;can't annotate a symbol, alas
     ((= expr)          (annotate! `(= ,$2)))
     ((LP pat RP)       $2)
     ((LP p-list* RP)   (annotate! `(cons ,@$2)))
     ((Id)              (annotate! `',$1))
     ((Id LP p-list RP) (annotate! (match $3
                                     ['()  `',$1]
                                     [pats `(',$1 ,@pats)]))))
    (p-list* (() '())
    ...
    (expr ((e) (annotate! $1)))
    (e
     ((LAMBDA names => expr)        (annotate! `(fn ,@$2 ,$4)))
     ((LET decls IN expr)           (annotate! `(let ,$2 ,$4)))
     ((CASE e-op cases)             (annotate! `(case ,$2 ,@$3)))
     ((IF e-op THEN e-op ELSE e-op) (annotate! `(if ,$2 ,$4 ,$6)))
     ((WHEN e-op THEN expr)         (annotate! `(when ,$2 ,$4)))
     ((UNLESS e-op THEN expr)       (annotate! `(unless ,$2 ,$4)))
     ((FIX name = expr)             (annotate! `(fix ,$2 ,$4)))
     ((FIX name : type = expr)      (annotate! `(as ,$4 (fix ,$2 ,$6))))
     ((⋁ LP loops RP expr)          (annotate! `(,@$3 lub ,$5)))
     ((FOR LP loops RP expr)        (annotate! `(,@$3 lub ,$5)))
     ((e-asc)                       $1))
    (e-asc
     ((e-asc : type)    (annotate! `(as ,$3 ,$1)))
     ((e-op)            $1))
    (e-op
     ;; for now, everything is left associative. TODO: precedence parsing.
     ((e-op oper e-app)      (annotate! `(,$2 ,$1 ,$3)))
     ((e-op IN? e-app)       (annotate! `(,$1 in? ,$3)))
     ((Id LP e-list RP)      (annotate! `(',$1 ,@$3)))
     ((Id)                   (annotate! `',$1))
     ((e-app)                $1))
    (e-app
     ((e-app e-atom)   (annotate! `(,$1 ,$2)))
     ((e-atom)         $1))
    (e-atom
     ((name)                            $1)
     ((lit)                             $1)
     ((EMPTY)                           'empty)
     ((LP expr RP)                      $2)
     ((LP expr BAR loops RP)            (annotate! `(,$2 ,@$4)))
     ((LP e-list* RP)                   (annotate! `(cons ,@$2)))
     ((LCURLY e-list RCURLY)            (annotate! `(set ,@$2)))
     ((LCURLY e-op BAR loops RCURLY)    (annotate! `((set ,$2) ,@$4)))
     ;; TODO: multiple-element map literals; map comprehensions?
     ((LCURLY : RCURLY)                 (annotate! `(map)))
     ((LCURLY e-kv-list1 RCURLY)        (annotate! `(map ,@$2))))
