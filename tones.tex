...
We wish to validate the following tone-weakening and substitution rules:

\begin{mathpar}
  \infer[weakening]
        {A^s \le B^t \quad \checks{\GG, \h{x}{A}{s}}{M}{C}}
        {\checks{\GG, \h{x}{B}{t}}{M}{C}}

  \infer[substitution]
        {A^s \le B^t
         \quad \infers{\GG_1}{N}{A}
         \quad \checks{\GG_2, \h{x}{B}{t}}{M}{C}}
        {\checks{\GG_1{}^s \wedge \GG_2}{M[N/x]}{C}}
\end{mathpar}
...
\todo{TODO: function subtyping rules}

\todo{What's the right way to prove $\Box(A \to B) \le \Box A \to \Box B$?}
