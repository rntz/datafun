...
\begin{mathpar}
  \begin{array}{rccl}
    ...
    ~|~ \mb{let}~x = e~\mb{in}~ m\\
    %% TODO: should "case" really be a normal/checking form?
    &&|& \ecase{e}{x}{m}{y}{n}
    \vspace{0.5em}\\
...
subsection~\ref{subsec:principles}. Can we prove these with our original,
...
\todo{TODO: Explain why/when we use toneless contexts. Explain $\phi^{\vec{t}}$
  notation for a context split into its types and its tones.}
...
\todo{TODO: I now think modal stripping is the wrong approach, at least for
  product types. With these rules, we have $\pcheck{(x,(y,z))}{(A \x \Box(B \x
    C))}{\tm}{x : A, y : B, z : C}$. But the \emph{most informative} result
  would be $\pcheck{(x,(y,z))}{(A \x \Box(B \x C))}{\tm}{x : A, y : \Box B, z :
    \Box C}$.}
...
Now we update the helper judgment $\casecheck{p}{m}{\GG}{A}{s}{C}$ to pass through $\GG$ to the pattern:
