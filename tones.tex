...
\todo{TODO: Isn't $\wedge$ greatest lower bound / product? Don't we have
...
The judgment $\pcheck{p}{A}{s}{\phi}$ corresponds to a \Pre-morphism $A^s \to 1
+ \phi$. It means that the pattern $p$, when it matches a value of type $A$,
uses that value at tone $s$ to produce a values for $\phi$'s variables.
%
\begin{mathpar}
  \infer{ }{\pcheck{\pwild}{A}{\tb}{\emptycx}}

  \infer{ }{\pcheck{x}{A}{\tm}{x : A}}

  \infer{\converts{s}{A}{A_1 + A_2} \\ \pcheck{p}{A_i}{t}{\phi}}
        {\pcheck{\einj{i}{p}}{A}{s \tc t}{\phi}}

  \infer{\converts{s}{A}{A_1 \x A_2} \\
         (\forall i)~{\pcheck{p_i}{A_i}{t_i}{\phi_i}}\\
         %% \pcheck{p_1}{A_1}{t}{\phi_1} \\
         %% \pcheck{p_2}{A_2}{u}{\phi_2} \\
         \phi_1,\phi_2~\text{disjoint}}
        {\pcheck{(p_1, p_2)}{A}{s \tc (t_1 \wedge t_2)}{\phi_1,\phi_2}}
\end{mathpar}
...

\subsection{Alternative: Modal distribution}

\newcommand{\mdist}[2]{#1 \equiv #2}
\newcommand{\anybinop}{\boxplus}

Consider matching a nested pair pattern $(x,(y,z))$ against a value of type $A
\x \Box(B \x C)$.
%
The rules above can derive:
\[\pcheck{(x,(y,z))}{(A \x \Box(B \x C))}{\tm}{x : A, y : B, z : C}\]

Although this is sound, the \emph{most informative} result would be:
\[\pcheck{(x,(y,z))}{(A \x \Box(B \x C))}{\tm}{x : A, y : \Box B, z : \Box C}\]

The problem lies in the interaction of modal stripping and the rule for pair
patterns $(p,q)$. So, rather than use modal stripping (or its cousin subtyping),
let's apply Theorem~\ref{thm:tones-monoidal} and distribute our modal operators
over $\x$ and $+$. We'll define judgments $A \equiv B \x C$ and $A \equiv B +
C$; in each, $A$ is an input and $B$, $C$ are outputs.

\begin{mathpar}
  \infer{ }{\mdist{A \x B}{A \x B}}

  \infer{\mdist{A}{B \x C}}{\mdist{\Box A}{\Box B \x \Box C}}

  \infer{\mdist{A}{B \x C}}{\mdist{\op\; A}{\op\; B \x \op\; C}}
\\
  \infer{ }{\mdist{A + B}{A + B}}

  \infer{\mdist{A}{B + C}}{\mdist{\Box A}{\Box B + \Box C}}

  \infer{\mdist{A}{B + C}}{\mdist{\op\; A}{\op\; B + \op\; C}}
\end{mathpar}

By distributing our modes we can define a simplifed, \emph{toneless} pattern
typing judgment $p : A \vdash \phi$:
%
\begin{mathpar}
  \infer{ }{\pwild : A \vdash \emptycx}
  ...
  \infer{
    \mdist{A}{A_1 + A_2} \\
    p : A_i \vdash \phi
  }{\einj{i}{p} : A \vdash \phi}

  \infer{
    \mdist{A}{A_1 \x A_2} \\
    (\forall i)~{p_i : A_i \vdash \phi_i}\\
    \phi_1,\phi_2~\text{disjoint}
  }{(p_1,p_2) : A \vdash \phi_1,\phi_2}
\end{mathpar}

Finally, our \mb{case} rule becomes:
\[
\infer{
  \infers{e}{\GG}{A} \\
  (\forall i)~{p_i : A \vdash \phi_i} \\
  (\forall i)~{\checks{m_i}{\GG_i,\phi_i^{\vec{s}_i}}{C}}
}{
...
\todo{However, I'm skeptical that this toneless approach will scale to nonlinear
  and guarded pattern matching. We could probably add tones back in, or we could
  understand guarded \& nonlinear pattern matching by as mere syntax sugar.}

Can we also use modal distribution instead of modal stripping in our rules for
expressions? Not quite. The tuple-projection rule can be rewritten:
\begin{mathpar}
  \infer{\infers{e}{\GG}{A} \\ \mdist{A}{A_1 \x A_2}}
        {\infers{\pi_i\;e}{\GG}{A_i}}
\end{mathpar}

However, the function application rule (shown below) cannot, because in general,
$\Box(A \to B) \not\equiv \Box A \to \Box B$. So it seems there is no choice but
to use subtyping.
...
Typing \mb{case} is complicated. I give two formulations of it, one direct and
one involving a helper judgment. Formulated as a single rule, it looks like
this:
%
\begin{mathpar}
  \infer{\infers{e}{\GG}{A} \\
...
which says that the \mb{case} branch $p \cto m$ uses variables in $\GG$, matches
a scrutinee of type $A$ that it uses at tone $s$, producing (if it matches) a
result of type $C$. This corresponds to a morphism $\GG \x A^s \to 1 + C$. Then
we have:
%
\begin{mathpar}
  \infer{\pcheck{p}{A}{s}{\phi} \\
    \checks{m}{\GG, \phi^{\vec{t}}}{C}}
  {\casecheck{p}{m}{\GG}{A}{s \tc \bigwedge\vec{t}}{C}}

  \infer{\infers{e}{\GG}{A} \\
    (\forall i)~~{\casecheck{p_i}{m_i}{\GG_i}{A}{s_i}{C}}}
        {\checks{\mb{case}~ e ~\mb{of} ~\xbar{i}{p_i \cto m_i}}
          {\GG^{\bigwedge_i s_i} \wedge \bigwedge_i \GG_i}{C}}
\end{mathpar}


\subsection{Patterns with guards}
