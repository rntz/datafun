...
\textbf{Abstract.}
%
  Many kinds of ``non-monotone'' maps (e.g.\! equivalence-preserving) can be
  understood as monotone, but with respect to an altered domain ordering. I call
  such alterations ``tones'', and give some basic results in tone theory.

  I sketch a $\lambda$-calculus where types are preorders and functions are
  monotone. Its type system incorporates tones, allowing selective use of
  non-mono\-tonicity. }
...
\todo{TODO: An intuition about $\ti$ and $\tb$: $\ti$ keeps only the
  ...
  connected components into \emph{strong} ones.}
...
words, if $A \subseteq B$ and $x \le y : A \implies x \le y : B$.) This lifts
pointwise to a partial order on tones: let $s \le t$ iff $A^s \le A^t$ for all
$A$.

As functors, tones also form a category, \Tone{}, with natural transformations
as morphisms. However, \Tone{} is a mere fa\c{c}ade over this partial
order:

\begin{theorem}\label{thm:tone-poset}
  $s \le t \iff \exists \eta : \Tone(s, t)$. Moreover, any two morphisms $\eta,
...
connective internalizing $-^s$. I aim to adapt this into a
