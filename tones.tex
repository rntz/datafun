...
\[\begin{array}{cccl}
  ...
  \vspace{0.5em}\\
  \text{pre-contexts} & \phi &\bnfeq& \emptycx ~|~ \phi,\, x : A
  \vspace{0.5em}\\
...
The judgment $\pcheck{p}{A}{s}{\phi}$ corresponds to a \Pre-morphism $\den{A}^s
\to 1 + \den{\phi}$. It means that the pattern $p$, when it matches a value of
type $A$, produces the pre-context $\phi$, and in doing so uses $A$ at tone $s$.
...
\begin{mathpar}
  ...
  \infer{\converts{s}{A}{A_1 \x A_2} \\
        ...
        {\pcheck{(p_1, p_2)}{A}{s \tc (t \wedge u)}{\phi_1,\phi_2}}
...
which says that the \mb{case} branch $p \cto m$ uses variables in $\GG$ to match
a scrutinee of type $A$, which it uses at tone $s$, producing (if it matches) a
result of type $C$. This corresponds to a morphism $\den{\GG} \x \den{A}^s \to 1 + \den{C}$. Then we have:
%
\begin{mathpar}
