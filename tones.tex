...
\renewcommand{\ta}{{\color{ForestGreen}\op}}   % antitone / op
\renewcommand{\ti}{{\color{NavyBlue}\iso}}     % invariant/ core
\renewcommand{\tb}{{\color{Bittersweet}\path}} % bivariant/ path
...
%% \renewcommand{\xbar}[2]{{}^{#1}\overline{#2}}
%% \renewcommand{\xbar}[2]{{}_{#1}\!\left(#2\right)}
\newcommand{\pwild}{\_}
...
\[\begin{array}{rccl}
  ...
  \vspace{0.5em}\\
  \text{toneless contexts} & \phi &\bnfeq& \emptycx ~|~ \phi,\, x : A
  \vspace{0.5em}\\
...

\subsection{Alternative: Modal distributing}
...
\x \Box(B \x C)$.
%
Applying the rules above derives:
...
patterns $(p,q)$. So, rather than use modal stripping, which is inspired by
subtyping, we could apply Theorem~\ref{thm:tones-monoidal} and distribute our
modal operators over $\x$ and $+$. Let's only consider $\x$ for now. Define the
judgment $A \equiv B \x C$, where $A$ is an input and $B$, $C$ are outputs, like
so:
%
\begin{mathpar}
...
Now, let's use this to rewrite our pattern typing rule for $(p,q)$:
\begin{mathpar}
  \infer{\mdist{A}{A_1 \x A_2} \\
         (\forall i)~\pcheck{p_i}{A_i}{s_i}{\phi_i} \\
         \phi_1, \phi_2~\text{disjoint}}
        {\pcheck{(p_1,p_2)}{A}{s_1 \wedge s_2}{\phi_1,\phi_2}}
\end{mathpar}
