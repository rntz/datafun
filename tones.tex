...
\usepackage[b5,width=365pt]{rntzgeometry}
...
\newcommand{\iso}{\ensuremath{\textrm{iso}}}     % iso, core, inv, eq
\renewcommand{\path}{\ensuremath{\textrm{path}}} % path, loc, biv, bi

\renewcommand{\iso}{\texorpdfstring{\ensuremath{\Box}}{iso}}
\renewcommand{\path}{\texorpdfstring{\ensuremath{\lozenge}}{path}}

% \kern0pt is a hack to make TeX not vertically center the argument if it's only
% a single character.
\newcommand{\toneof}[2]{#1 #2}
%% \newcommand{\idof}[1]{\toneof{\id}{#1}}
%% \newcommand{\opof}[1]{\toneof{\op}{#1}}
%% \newcommand{\isof}[1]{\toneof{\iso}{#1}}
%% \newcommand{\pathof}[1]{\toneof{\path}{#1}}
\newcommand{\idof}{\id\,}
\newcommand{\opof}{\op\,}
\newcommand{\isof}{\iso}
...
%% \renewcommand{\ta}{{\op}}   % antitone / op
%% \renewcommand{\ti}{{\iso}}     % invariant/ iso
%% \renewcommand{\tb}{{\path}} % bivariant/ path
...
\begin{figure*}
  \begin{mathpar}
    \begin{array}{lr@{\hskip 0.5em}c@{\hskip 0.5em}l}
      ...
      \tb~\text{is identity} & \tb \tmeet T &=& T\\
      \ti~\text{absorbs} & \ti \tmeet T &=& \ti\\
    \end{array}
    ...
    \begin{array}{lr@{\hskip 0.5em}c@{\hskip 0.5em}l}
      ...
      \text{Identity} & \multicolumn{3}{c}{\tm \tc T = T = T \tc \tm}\\
      \tb~ \text{left-absorbs} & T\tb &=& \tb\\
      \ti~ \text{left-absorbs} & T\ti &=& \ti\\
      \ta~ \text{involutive} & \ta \tc \ta &=& \tm\\
    \end{array}
...
%
\begin{eqnarray*}
  a \le b : \toneof{(T \tmeet U)}{A} %% \iff a \le b : A^s \cap A^t
...
\begin{mathpar}
  ...
  \infer[Hypothesis]{T \le \tm}{\extend{\GG}{\mtp{T}{A}} \vdash A}
...
\begin{mathpar}
  \begin{array}{rcclr}
    ...
    \text{tones} & T,U,V & \bnfeq & \tm \pipe \ta \pipe \tb \pipe \ti
    \vspace{1pt}\\
...
\begin{mathpar}
  ...
  %% lambdas
  \infer{\checks{m}{\extend{\GG}{\h{x}{T}{A}}}{B} \\ \tm \le T}
...
UVA \to A$, thus $UVA \le A$, and so finally $TUVA \le TA \le B$. \todo{Clean up
  this explanation. Explain that we use adjunction rather than $st \le \tm$
  directly because adjunction gives us the \emph{most informative} result; $st
  \le \tm$ is declarative, $\adjoint{t}{s}$ is algorithmic. Give explanations
...
%
\begin{mathpar}
  \infer{\tm \le T \\ \mtp{T}{A_2} \subtype A_1 \\ \mtp{\tm}{B_1} \subtype B_2}
        {\mtp{\tm}{A_1 \to B_1} \subtype A_2 \to B_2}

  \infer{\ta \le T \\ \mtp{T}{A_2} \subtype A_1 \\ \mtp{\ta}{B_1} \subtype B_2}
        {\mtp{\ta}{A_1 \to B_1} \subtype A_2 \to B_2}

  \infer{\tb T = \tb \\ \mtp{T}{A_2} \subtype A_1 \\ \mtp{\tb}{B_1} \subtype B_2}
        {\mtp{\tb}{A_1 \to B_1} \subtype A_2 \to B_2}

  \infer{\mtp{\tb}{A_2} \subtype A_1 \\ \mtp{\ti}{B_1} \subtype B_2}
        {\mtp{\ti}{A_1 \to B_1} \subtype A_2 \to B_2}
\end{mathpar}

The premise $\tb T = \tb$ of the third rule holds for $T \ne \ti$ in our system;
however, $\tb T = \tb$ captures more exactly \emph{why} the rule is valid.
...
\todo{Are there also more precise/suggestive versions of the other premises? Can
  the $U \le T$ constraints be turned into ``composing with $V$ is $\ge \tm$'',
...
useful:
\[ \infer{P~\text{equiv}}{\mtp{\tb}{P} \subtype P} \]
...
\begin{mathpar}
  \infer*{\infer*{\vdots}
           {{\extend{\GG}{\h{x}{\ti}{A}}} \vdash {m} : {B}}}
         {{\GG} \vdash {\fnof{x}{m}} : {\T{\ti} A \to B}}
\end{mathpar}

But this \emph{could} check as follows:
\begin{mathpar}
  \infer*{\infer*{\vdots}{\checks{m}{\extend{\GG}{\h{x}{T}{(\T{\ti} A)}}}{B}}
          \\ \tm \le T}
         {\checks{\fnof{x}{m}}{\GG}{\T{\ti} A \to B}}
\end{mathpar}

So the crucial question is: can we always substitute $\h{x}{\tm}{\T{\ti} A}$
for $\h{x}{\ti}{A}$?
%
...
\begin{mathpar}
  \infer{x \not\in \phi}{
    \pcheckio{x}{\tm}{A}{\mprecx{\vv{\tb}}{\phi}}{x : A}}
  ...
  \infer{\mdist{A}{B \x C} \\
    ...
    \pcheckio{(p,q)}
             {\left(\tm \tmeet \bigmeet\!\vv{V}\right) T \tmeet U}
             {A}
             {\mcx{\tm \tmeet \bigmeet\!\vv{V}}{\GG_1} \tmeet \GG_2}
             {\phi, \psi}}
  ...
  \infer{\pcheckio{p}{T}{A}{\GG_1}{\phi} \\
    ...
    \pcheckio
        {\pif{p}{m}}
        %{\left(\tm \tmeet \tb{\bigmeet\!\vv{U}} \right) T}
        {T \tmeet \tb\bigmeet\!\vv{U}T}
        {A}
        {\mcx{\tm \tmeet \tb\bigmeet\!\vv{U}}{\GG_1} \tmeet \mcx{\tb}\GG_2}
        {\phi}}
\end{mathpar}
