...
%% \usepackage[laptop]{rntzgeometry}
%% \usepackage[width=345pt,fullwidth=370pt]{narrow}
%% \pagestyle{empty}
%% \usepackage[scaled=0.95]{rntzfont}

% Options: charter, palatino, cochineal, libertine.
%\usepackage[b5]{rntzgeometry}\usepackage[width=365pt]{narrow}
...
\begin{theorem}[Tones distribute over $\x$ and $+$]\label{thm:tones-monoidal}
  \(T(A + B) = TA + TB\) and \(T(A \x B) = TA \x TB\).
\end{theorem}
...
%% A good counterexample is the
%% \textsf{fencepost} preorder on $\N$, where $a < a+1$ for even $a$, and $a > a+1$
%% for odd $a$:
