...
\footnotetext{Indiscreteness (and its dual, discreteness) are also tones ---
  ...
  posets, so many intuitions transfer from one to the other.}
%
\begin{mathpar}
...

\subsection{About the $\fn$ rule}
I think the $\fn$ rule is justified, but a more obviously correct rule would be
something like:

\begin{mathpar}
  \infer{\checks{\GG{},\, \h{x}{A}{s}}{M}{B} \quad A \sqsubset A^s}
        {\checks{\GG}{\fn\bind{x}{M}}{A \to B}}
\end{mathpar}

I don't have a judgment for $A \sqsubset B^s$, where $A$,$B$,$s$ are all inputs.
I probably could. But is it necessary?

Actually, it's not clear to me I need to mess around with tones in $\fn$ at all.
The reason I thought we did was to justify something like the following:
\begin{mathpar}
  \infer{{\GG{},\, \h{x}{A}{\ti}} \vdash {M} : {B}}
        {{\GG} \vdash {\fn\bind{x}{M}} : {\Box A \to B}}
\end{mathpar}

But actually, this will check as follows:
\begin{mathpar}
  \infer{\checks{\GG{},\, \h{x}{\Box A}{s}}{M}{B} \quad \tm \le s}
        {\checks{\GG}{\fn\bind{x}{M}}{\Box A \to B}}
\end{mathpar}

So the crucial question is: \emph{can we always substitute $\h{x}{\Box A}{\tm}$
  for $\h{x}{A}{\ti}$?}

More generally, \emph{what is the substitution principle here, and how do we
  prove it?} It should have something to do with a subtyping rule, like
$\subtype{s}{A}{B}$. Only probably I want a declarative subtyping rule with
tones on both sides; $A^s <: B^t$ or similar.
