\documentclass[b5,wide,libertine,ribbons]{rntz}
...
\begin{mathpar}
  \begin{array}{cccl}
    ...
    ~|~ \mb{let}~x = N~\mb{in}~ M
    \vspace{0.5em}\\
    %% TODO: use more distinct variables for checking/inferring terms
    %% v for checking forms, t for inferring terms?
...
Most complex are the four rules for function subtyping, $\subtype{s}{(A_1 \to
  B_1)}{A_2 \to B_2}$, one for each tone $s$, determined by the result of
$\subtype{s}{B_1}{B_2}$.
...
\todo{TODO: Can we unify \textsc{weakening} and \textsc{subtyping left}?}
