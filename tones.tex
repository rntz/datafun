...
Most complex are the four rules for function subtyping, $\subtype{s}{(A_1 \to
  B_1)}{A_2 \to B_2}$, one for each tone $s$ given by the result of
$\subtype{s}{B_1}{B_2}$:
%
\begin{mathpar}
...
\todo{TODO: Explain why/when we use toneless contexts. Explain $\phi^{\vec{t}}$
  notation for a context split into its variables and its tones.}
...
produces the pre-context $\phi$, and in doing so uses $A$ at tone $s$.

This interpretation cannot handle non-linear patterns, or patterns with guard
expressions; for that we'd need to give the morphism access to an input context
$\GG$.
