...
\begin{proof}
  ...
  \mb{1} \to A = \fnof{\star} x$. Then by naturality of $\eta$ the following
  square commutes:
  %
  \[\tikzset{
...
From Definition~\ref{def:tone} we have $f^s = f = f^t$; and since $\mb{1}$ is
terminal, we have $\eta_{\mb{1}} = {!} = id$; thus:
  \[\begin{array}{rlcl}
