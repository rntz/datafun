...
\todo{\todo{FIXME}: this doesn't type check! In $\downelim{x}{e} \letv{dx}{\mb{0}\;
...
\[\begin{array}{rcl}
  ...
  &&|\quad \forall(x \in \Den{A})~ \forall(da \in \GD_A(a))~ df(a)(da) \in \GD_B(f(a))\\
  &&{\!\!\wedge}~~ (f \oplus_{A \to B} df) ~\textsf{is monotone} ~\}
...
\todo{FIXME: NOT WELL-FOUNDED. Because $\oplus_A : \Pi(a : A)~ \GD_A(a) \to A$
  ...
  probably be fixed by eta-expanding a little?}
...
\item Define $\oplus_A : a : \Disc{\Den{A}} \to \GD_A(a) \to \Den{A}$ and show it
  is monotone in its second argument. This also involves defining \mb{0}.
\end{enumerate}
