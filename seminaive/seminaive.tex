...
\begin{mathpar}
  ...
  \infer{\fixle{x}{e}{f} : \fixletype{A}}{
    e : \fixletype{A} &
    \J{\GP}{\GG,x \of \fixletype{A}}{f}{\fixletype{A}}}
\end{mathpar}
...
\todo{FIXME: these operators don't typecheck! note this and call them
  pseudo-operators? Or maybe $\oplus$ should be discrete in its first argument?}
\todo{FIXME: $\oplus$ uses $\mb{0}$!}
...
\begin{array}{lcll}
  ...
  && \text{\todo{FIXME: next one needs a \textsf{split-sum} somewhere}}\\
  && \text{\todo{TODO: use dummy-value function}}\\
  ...
  \delta(\downelim{\d{x}}{e}{f}) &=& \text{\TODO}\\
  \delta(\etrue) &=& \efalse\\
  \delta(\efalse) &=& \efalse\\
  \delta(\when{e}{f})
