\RequirePackage{silence}\WarningFilter{amsmath}{Foreign command}
...
completeness at $A$ and $B$ and the axiom of choice. \todo{TODO: This is
  non-obvious; even I'm not convinced by this handwave.}
...
\begin{proof} \label{proof:correct}
  ...
  \item[Case \(\fnof x e\):] Then \(\phi(\fnof x e) = \fnof x \phi e\) and
    ...
    \item We wish to show that if $\longvld{\dgamma}\g{\g'}$ then
      %% FIXME: this runs off the page!

      \begin{align*}
        %% {\den{\fnof x \phi e} \<\g}
        %% &\longvalid{\den{\fnof{\eboxd x} \fnof{\dx} \delta e} \<\extend\g{\dgamma}}
        %% {\den{\fnof x \phi e} \<\g'}
        %% \\
        a \mapsto \den{\phi e}\<\extend{\g}{x \substo a}
        &
...
%
  ...
  \item[Case \(\eisEmpty e\):] Note that $\phi(\eisEmpty e) = \delta(\eisEmpty
    ...
    $\delta(\eisEmpty e)$. \todo{TODO: clarify}
    ...
    \item Assume $\g \approx \rho$. We wish to show that $\den{\phi(\eisEmpty
      ...
      \den{e}\<(\strip\<\rho)$. \todo{TODO: finish this using lemmas that (1)
        $\Phi(\eqt A) = \eqt A$ and (2) $x \approx_{\eqt A} y \iff x = y$.}
    ...
    \item We wish to show that, if $\longvld\dgamma\g{\g'}$ then
      ...
      does not use any derivative variables. \todo{This is a handwave.} So in
      ...
      Luckily, this is true of \emph{all} changes at this type: $\fa{x : \termO
        + \termO : \CP} \longvld{x}{x}{x}$. \todo{TODO explain}
