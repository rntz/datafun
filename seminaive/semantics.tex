%\documentclass[12pt]{rntz}\usepackage[a4]{rntzgeometry}\usepackage[fullwidth=18cm,width=420pt,width=38em,]{narrow}
...
% Allow re-statable theorems. Consider integrating into rntzthm.
...
correct by relating it to the incremental semantics. \XXX
...
\begin{figure*}
  \figsectionname{Types and contexts}\vspace{-.5ex}
  ...
  \vspace{1ex}\figsectionname{Terms}\vspace{-.5ex}
  ...
  \vspace{1ex}\figsectionname{Lemmas}\vspace{-.5ex}
...
\noindent
Product- and sum-distribution become identity maps, because $\prod_i \altiso A_i
= \altiso \prod_i A$ and $\sum_i \altiso A_i = \altiso \sum_i
A_i$.\footnote{Checking this is left as an exercise for the reader.}

$\altiso$ avoids needing the non-constructive $\zero_x$ in its comonad
extraction $\varepsilon$. However, for our purposes this turns out not to be an
issue, while the fact that $\chgs \isof A = \termO$ will be crucial.
\todo{explain why?}
...
definable via the following translation (also named $\D$, at the risk of
possible semantic overload):
...
computes the \emph{derivative} of $\phi e$. The types of these expressions are
related to $e$ as follows:

\begin{restatable}[Type-correctness]{theorem}{thmTypeCorrect}
  \label{thm:type-correct}
  If $\J e \G A$, then
  \[ \J {\phi e} {\Phi\G, \Delta\Phi\stripcxd\G} {\Phi A}
  \qquad\text{and}\qquad
  \J {\delta e} {\XXX} {\Delta \Phi A}
  \]
\end{restatable}
\begin{proof}
...
\begin{figure*}
  ...
  \figsectionname{Interesting Cases}\vspace{-.5ex}
  ...
  \vspace{1ex}\figsectionname{Distributive Cases}\vspace{-.5ex}
...
\begin{figure*}
  ...
  \begin{align*}
    ...
    %% TODO: move these up to two-equations-per-line figure?
    \delta(\eisEmpty e) &= \XXX &
    \delta(\esplit e) &= \esplit \delta e
  \end{align*}
