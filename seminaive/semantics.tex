...
\item $\vals A \in \Poset$ is the poset of values.
  %% \todo{TODO: hold on, why does
  %% this need to be a poset? Why not just a set, and then \emph{recover} the
  %% partial order (argh, actually a \strong{preorder}) from the update map? well,
  %% because I want the forgetful functor down to \Poset{} to preserve all relevant
  %% structure so my correctness proof works. But this is no big deal, I'll just
  %% have to prove \emph{that} as I go along rather than proving soundness \&
  %% completeness. But hold on, how do I define morphisms then?}
