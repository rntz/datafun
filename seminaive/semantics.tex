...
\begin{figure*}
  ...
  \todo{TODO: add a function $\morph{box}(f) = \morph{strip} \then \isof f$, so
...
\label{sec:CP-semilattice-objects}
...
$\longvld{d\gamma}{\gamma}{\gamma'} : \G$. From this we have:
...
\noindent
We lift this from types to contexts as follows: \todo{(TODO: notation)}

\nopagebreak[2]
\begin{align*}
  {\impls_\G} &\subseteq \den{\Phi\G, \D\Phi\stripcxd\G} \x \den{\G}
  \\
  \gamma \impls_\G \gamma'
  &\iff \fa{\h x A \in \G} \gamma_x \impls \gamma'_x\\
  &\phantom{\iff} \wedge \fa{\hd x A \in \G}
  \gamma_{\dvar x} \impls \gamma'_{\dvar x}
  \wedge \longvld{\gamma_{\dvar \dx}}{\gamma_{\dvar x}}{\gamma_{\dvar x}}
\end{align*}

\noindent
\todo{TODO: lift to open terms}

\begin{restatable}[Correctness]{theorem}{thmCorrect}
  $\den{\phi e} \impls \den{e}$ and
  $\longvld{\den{\delta e}}{\den{\phi e}}{\den{\phi e}}$.
\end{restatable}
\begin{proof}
  \todo{TODO}
\end{proof}
...
\renewcommand\qed{\vspace{\baselineskip}\hrule\vspace{.5\baselineskip}}         %FIXME hack.
...
\begin{proof}\label{proof:forget}
  ...
  \item[Case $\eboxd e$]
    \begin{align*}
      ...
      &= \vals(\morph{strip} \then \isof{\incdens e})\\
      &= \morph{strip} \then \isof{\vals\incdens e}
      && \text{by \cref{lem:vals-strip}}\\
      &\stareq \morph{strip} \then \isof{\den e} && \text{by IH}\\
      &= \den{\eboxd e}
    \end{align*}
...
\newcommand\phicx[1]{\Phi{#1},\DP\stripcxd{#1}}
...
\begin{proof}\label{proof:typeCorrect}
  ...
  \item[Case $x$ (monotone):] $\phi x = x$ and $\delta x = dx$. Since $\h x A
    ...
    %
    \begin{mathpar}
      \infer{\h x {\Phi A} \in \phicx\G}{\J x {\phicx\G} {\Phi A}}
  ...
  \item[Case $\dvar x$ (discrete):] $\phi{\dvar x} = \dvar x$ and $\delta{\dvar
    ...
    and $\dvar\dx \in \DP\G$, so by weakening:
    %
    \begin{mathpar}
      \infer{\hd x {\Phi A} \in \phicx\G}{\J {\dvar x} {\phicx\G} {\Phi A}}
    ...
    \begin{mathpar}
      \infer*{
        \infer*[fraction={===},leftstyle={},Left={rewrite context}]{
          \deduce{\textit{by IH}}{\J{\phi e} {\phicx{(\G,\h x A)}} B}
        }{\J {\phi e} {\phicx{\G}, \h x {\Phi A}} {\Phi B}}
      }{\J {\fnof x \phi e} {\phicx\G} {\Phi A \to \Phi B}}
