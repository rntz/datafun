...
zero-change at $\pfinof\termO$ by \cref{eqn:CP-P-upd}; to see why $\bot$ is a
...
\begin{proof}\label{proof:eqsemi}
  ...
  \item[Case $\eqt L \x \eqt M$:] \(\den{\eqt L \x \eqt M} = \den{\eqt L} \x
    ...
    \(\vals{\den{\eqt M}} = \chgs{\den{\eqt M}}\) and both are semilattices.
    The product of semilattices is a semilattice itself, and we have:
    \begin{align*}
      \tuple{x,y} \upd_{\den{\eqt L} \x \den{\eqt M}} \tuple{\dx,\dy}
      &= \tuple{x \upd \dx,\, y \upd dy}\\
      &= \tuple{x \vee \dx,\, y \vee dy} & \text{by IH}\\
      &= \tuple{x,y} \vee \tuple{\dx,\dy}
    \end{align*}

  \item[Case $\tseteq A$:] \XXX
  \end{description}
\end{proof}
