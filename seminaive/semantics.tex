...
\noindent
Product- and sum-distribution are actually identity maps, because $\prod_i
...
$\altiso$ avoids the need to use the non-constructive $\zero_x$ to define the
comonad extraction $\varepsilon$. However, for us this will turn out not to be
an issue. \todo{TODO: explain why $\chgs \isof A = \termO$ turns out to be handy
  - it allows us to split up $\Phi$ and $\Delta$ on types.}
...
\begin{restatable}{theorem}{forgetInc}
  \label{thm:forget-incremental}
  \(\vals{\incden{-}} = \den{-}\).
\end{restatable}

\begin{proof}
  By induction on types and typing derivations. 
  For example,
  ...
  \noindent See \cpageref{proof:forget-incremental} for full proof.
\end{proof}
...
\clearpage
\begin{center}
  \Large \itshape Appendix: Proofs
\end{center}

\forgetInc*
\begin{proof}\label{proof:forget-incremental}
  Induct on types and typing derivations. We omit the proof for types; it
  follows directly from the construction in \cref{sec:changeposets}. For typing
  derivations:
