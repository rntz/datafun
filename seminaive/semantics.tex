...
\item Booleans, which in Datafun are \emph{not} simply a sum of units, because
  ...
  with $\efalse = \emptyset$ and $\etrue = \eset{\etuple{}}$. \todo{TODO:
    explain $(\eisEmpty e)$}.

\item Pattern matching on tuples and boxes. \todo{TODO: explain $\prim{split}$.}

\item In the core language, \kwname{for}-loops are limited to only first-order
  semilattice types $\eqt L$; this simplifies the semantics and semi\naive{}
  transformation. However, uses for \kwname{for} at higher-order types can be
...
recall that for any semilattice object $A \in \CP$, we have $\chgs A = \vals A$
and $x \upd_A y = x \vee_{\vals A} y$. Now, suppose
