...
\begin{restatable}[Correctness]{theorem}{thmCorrect}\label{thm:correct}
  If $\J e \G A$ and $\gamma \impls_\G \rho$ and
  $\longvld{\dgamma}{\gamma}{\gamma'} : \incdens{\Phi\G}$, then

  %% \begin{enumerate}
  %% \item \(\den{\phi e} \<\gamma \impls_A \den{e} \<\rho\)
  %% \item \(\longvld{\den{\delta e} \tuple{\gamma,\dgamma}}{\den{\phi e} \<\gamma}{\den{\phi e} \<\gamma'}\)
  %% \end{enumerate}
...
\begin{proof} \label{proof:correct}
  ...
  \item[Case \((\elet{\eboxd {x : A} = e} f)\):] From
    ...
    \noindent
    From our semantics and \cref{lem:letboxtuple} we have
    \begin{align*}
      ...
      &= \den{\phi f} \<\extend{\g}{
        \dvar x \substo \pi_1(\den{\phi e}\<\g),\,
        \dvar\dx \substo \pi_2(\den{\phi e}\<\g)}
      \\
      \den{\elet{\eboxd{\etuple{x,\dx}} = \phi e} \delta f}\<\g
      &= \den{\delta f} \<\extend{\g}{
        \dvar x \substo \pi_1(\den{\phi e}\<\g),\,
        \dvar\dx \substo \pi_2(\den{\phi e}\<\g)}
    \end{align*}
    ...
    \item We want to show:
      ...
      \noindent
      By our IH for $f$, it suffices to show that
      \[
      \extend{\g}{
        \dvar x \substo \pi_1(\den{\phi e}\<\g),\,
        \dvar\dx \substo \pi_2(\den{\phi e}\<\g)}
      \approx
      \extend{\rho}{\dvar x \substo \den{e}\<\rho}
      \]

      By our IH for $e$ we have $\den{\phi e}\<\g \approx_{\isof A}
      \den{e}\<\rho$, and therefore $\pi_1\<(\den{\phi e}\<\g) \approx_A
      \den{e}\<\rho$ and $\longvld{\pi_2\<(\den{\phi e}\<\g)}{\pi_1\<(\den{\phi
          e}\<\g)}{\pi_1\<(\den{\phi e}\<\g)}$. Combined with $\gamma \approx
      \rho$, this suffices.
    ...
    \item We wish to show that
      ...
      \longvld
          ...
          {\den{\phi f} \<\extend{\g'}{
              ...
              \dvar\dx \substo \pi_2(\den{\phi e}\<\g')}}
      \]
      \todo{darn, this doesn't even fit on the page}
      \XXX
    \end{enumerate}
