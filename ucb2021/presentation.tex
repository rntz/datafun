%% TODO: aspect ratio
\documentclass[aspectratio=169,dvipsnames,draft]{beamer}
...
%\setbeamerfont{frametitle}{family=\displayfamily}
...
\subtitle{bottom-up logic \,$\color{Gray}\bowtie$\, \color{black}higher-order functions%\\ \color{Gray}incrementally%
}
\author{\itshape michael \color{black}\upshape\scshape arntzenius}
%\author{\itshape michael \color{black} arntzenius}
%\author{\scshape michael \color{black} arntzenius}
\date{}
...

  \begin{frame}{}
    ...
    \todo{comma instead of wedge?}
    \begin{fleqn}[1em]
    \begin{minipage}[t][.29\paperheight]{\paperwidth}
      \[\begin{array}{l}
      \n{reach}(\c{start}).\\
      \n{reach}(Y) \gets \n{reach}(X) \wedge \n{edge}(X,Y).\\[1ex]\pause
      \n{reach2}(\c{start}).\\
...

  \begin{frame}
    ...
    \begin{fleqn}
      ...
      \n{reachable} \;\n{start} \;\n{edge} = \pause
      %% \n{reach} \;(\n{start}\!: \tnode) \;(\n{edge}\!: \tset(\tnode \x \tnode))
      %% \!: \tset(\tnode)
      %% = \pause
%      \kw{fix}~ S ~\kw{is}~ \pause \{\n{start}\} \cup \setfor{y}{x \in S, (x,y) \in \n{edge}}
      \\\quad
...

  ...
  \begin{frame}
    ...
    \item a pure functional language \pause
    \item with \setcolor{sets}
      \& \compcolor{set/lattice comprehensions} \pause
...

  \begin{frame}{\scshape monotonicity}
    \centering
...

  \begin{frame}{\scshape lattice comprehensions}
    %% \todo{relational composition}\\
    %% \todo{exists as example of lattice aggregation?}
...

  %% \begin{frame}{\scshape cyk parsing}
...

  \begin{frame}%{\scshape change in datafun}
