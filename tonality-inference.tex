...
$C^\core$ and $C^\loc$ are always groupoids --- categories where every morphism
is an isomorphism. I assert but do not prove that $-^\core$ and $-^\loc$ are
...
\mb{Cat}
  \arrow[r,bend left=35,"\core"]
  \arrow[r,bend right=35,"\loc"']
& \mb{Groupoid} \lar
\\
\mb{Preorder}
  \arrow[r,bend left=35,"\core"]
  \arrow[r,bend right=35,"\loc"']
  \arrow[u]
& \mb{Setoid} \arrow[u] \arrow[l]
\end{tikzcd}\]

\todo{Unanswered questions:}
\begin{eqnarray*}
  \mb{Gpd}(A^\core, B) &{\cong}?& \mb{Cat}(A, \mc{U}B)
  \qquad\text{\color{red} unlikely!}\\
  \mb{Cat}(\mc{U}A, B) &{\cong}?& \mb{Grpd}(A, B^\core)
  \qquad\text{\color{ForestGreen} this looks promising!}\\
  \mb{Grpd}(A^\loc, B) &{\cong}?& \mb{Cat}(A, \mc{U}B)
  \qquad\text{\color{blue} maybe?}\\
  \mb{Cat}(\mc{U}A, B) &{\cong}?& \mb{Grpd}(A, B^\loc)
  \qquad\text{\color{red} I don't think so...}
\end{eqnarray*}

\todo{I think \loc{} and \core{} form an adjoint triple!}
