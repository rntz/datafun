...
\item Meet $s \wedge t$ is the meet of the lattice ordered $\ti < \{\tm, \ta\} <
  \tb$. This gives the general tone of the pairing of two functions: given $f :
  A \overset{s}{\to} B$ and $g : A \overset{t}{\to} C$, we have $\langle f,
  g\rangle : A \overset{s \wedge t}{\to} B \times C$.

\item Composition $s \tc t$ gives the general tone of a composed function $f
  \circ g$ when $f : B \overset{s}{\to} C$ and $g : A \overset{t}{\to} B$;
  equivalently, $T_{s \tc t}\, A = T_t\,T_s\, A$ (note that the relative order
  of $s,t$ swaps in this formulation).
\end{enumerate}
...
\begin{mathpar}
  ...
  \begin{array}{lr@{\hskip 0.5em}c@{\hskip 0.5em}l}
    \textbf{Left-distribution} & s \tc (t \vee u) &=& (s \tc t) \vee (s \tc u)\\
    \textbf{Right-distribution} & (s \vee t) \tc u &=& (s \tc u) \vee (t \tc u)
  \end{array}
\end{mathpar}
