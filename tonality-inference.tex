...
as ``addition'' and $\tc$ as ``multiplication''. (\todo{TODO: Reference ``I Got
  ...
  annotations in the inference rules.}) This is because the following panopoly
...
\begin{mathpar}
  \begin{array}{lr@{\hskip 0.5em}c@{\hskip 0.5em}l}
    ...
    \textbf{\color{ForestGreen} Idempotence} & s \vee s &=& s\\
  ...
  \begin{array}{lr@{\hskip 0.5em}c@{\hskip 0.5em}l}
    ...
    \textbf{\color{ForestGreen} Commutativity} & s \tc t &=& t \tc s\\
...

\section{As a sequent calculus}

Let $F_t$ be the type constructor
corresponding to the tone $t$; so:
\begin{eqnarray*}
  F_{\tm} A &=& A\\
  F_{\ta} A &=& A^{\op}\\
  F_{\ti} A &=& \square{} A\\
...
\begin{mathpar}
  (A_1^{t_1}, ..., A_n^{t_n}) = (A_1^{t_1 \tc u}, ..., A_n^{t_n \tc u})\\

  \infer{\GG \vdash A}{\GG^t \vdash F_t A}

  \infer{\GG,A^{t\tc u} \vdash C}{\GG, (F_t A)^u \vdash C}
