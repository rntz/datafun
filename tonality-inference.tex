...
\newcommand{\subtype}[3]{{#2}\ {#1}<: {#3}}
\newcommand{\isfn}[4]{{#2}\ {#1}<: {#3} \Rightarrow {#4}}
...
\begin{mathpar}
  \infer{\checks{\GG\,s}{M}{A}}{\infers{\GG\,s}{M : A}{A}}

  \infer{\infers{\GG\,s}{N}{A} \quad \subtype{t}{A}{B}}
        {\checks{\GG\,s \tc t}{N}{B}}

  \infer{\quad}{\infers{\GG\,\tb,~x : A\,\tm}{x}{A}}

  %% functions & function application
  %% TODO: need ``coerce to a function'' subtyping rule!
  \infer{\checks{\GG\,s,~x : A\,t}{M}{B} \quad \tm \le t}
        {\checks{\GG\,s}{\fn\bind{x} M}{A \to B}}

  \infer{\infers{\GG\,s}{N}{A}
         \quad \isfn{u}{A}{B}{C}
         \quad \checks{\GG\,t}{M}{B}}
        {\infers{\GG\,((s \tc u) \wedge t)}{N\; M}{C}}

  %% pairs
  \infer{\checks{\GG\,s}{M_1}{A_1} \quad \checks{\GG\,t}{M_2}{A_2}}
        {\checks{\GG\,(s \wedge t)}{(M_1,M_2)}{A_1 \x A_2}}

  \infer{\infers{\GG\,s}{N}{A_1 \x A_2}}
        {\infers{\GG\,s}{\pi_i\;N}{A_i}}

  %% type-driven subtyping for checking
  \infer{\checks{\GG\,s}{M}{A}}
        {\checks{\GG\,s \tc \ti}{M}{\Box A}}

  \infer{\checks{\GG\,s}{M}{A}}
        {\checks{\GG\,s \tc \ta}{M}{\op\; A}}
\end{mathpar}
...

\subsection{Function application subtyping}

\begin{mathpar}
  \infer{\quad}{\isfn{\tm}{A \to B}{A}{B}}

  \infer{\isfn{s}{A}{B}{C}}
        {\isfn{\tb \tc s}{\Box A}{B}{C}}

  \infer{\isfn{s}{A}{B}{C}}
        {\isfn{\ta \tc s}{\op\; A}{\op\;B}{\op\;C}}
\end{mathpar}
