...
\begin{center}
  \begin{tabular}{cll@{\hskip 0.25em}c@{\hskip 0.25em}l}
    ...
    \\\hline
    \tm & \text{ordering}
    & $a \le b : A^\tm$ & $\iff$ & $a \le b : A$
    \\
    \ta
    & \text{opposite ordering}
    & $a \le b : A^\op$ & $\iff$ & $a \ge b : A$
    \\
    \tb{}
    & \text{equivalence closure or ``paths''}
    & $a \le b : A^\path$ &$\Longleftarrow\ $&
    $a \le b \vee b \le a : A$ \emph{\small (see note)}$^\dagger$\\
    \ti
    & \text{induced equivalence or ``core''}
    & $a \le b : A^\iso$ & $\iff$ & $a \le b \wedge b \le a : A$
  \end{tabular}
\end{center}
...
\subsubsection{$\dagger$ Note on defining $\path$}

$A^\path$ is the smallest preorder satisfying
\[ a \le b \vee b \le a : A \implies a \le b : A^\path \]

Recall that $a \le b: A^\iso$ iff $a \le b \wedge b \le a : A$.
%
Unfortunately, $A^\path$ is not so easy to define \emph{directly}, without
language like ``the smallest preorder satisfying''.
%
In particular, $a \le b \vee b \le a : A$ implies $a \le b : A^\path$, but the
reverse does not always hold. A good counterexample is the \ms{fencepost}
ordering:

\todo{TODO: Diagram}.

That is, $a \le a+1$ for even $a$, and $a \ge a+1$ for odd $a$. In
$\ms{fencepost}^\path$, everything is equivalent:
\[ 0 \equiv 1 \equiv 2 \equiv 3 \equiv 4 \equiv ... \]

But $a \le b \vee b \le a : \ms{fencepost}$ is equivalent to $|a-b| \le 1$,
which isn't even transitive!
...
\ge a_{i+1} : A$. Then $a \le b : A^\path$ if there is a path starting from $a$
and ending at $b$.

%% \begin{enumerate}
%% \item $A^{\ms{id}}$ is just $A$, unchanged.
%% \item $A^\op$ is $A$ with the opposite order: \[ a \le b : A^\op \iff a \le b : A \]
%% \item $A^\path$ gives the equivalence closure --- the smallest preorder such that
%%   \[(a \le b \vee b \le a) : A \implies a \le b : A^\path\]
%% \item $A^\iso$ gives the induced equivalence --- the smallest preorder such that
%%   \[(a \le b \wedge b \le a) : A \implies a \le b : A^\iso \]
%% \end{enumerate}

%% Another view of tones is as transformations $-^s : \mb{Preorder} \to
%% \mb{Preorder}$ which modify a preordered set's ordering without changing its
%% elements, namely:
