...
previously. The results are shown in \cref{fig:seminaive-vs-naive-graph}. The
$\phi$ transform alone (\emph{semi\naive\ raw}) provides only a small speed-up,
roughly \todo{15--20\%}. Only when followed by other optimizations
...
speedup.\footnote{We also tried following the $\phi$ transform with only
  ...
  essentially the same results as \emph{semi\naive\ full}, so we have omitted
  it from \cref{fig:seminaive-vs-naive-graph}. It is unclear to us whether
  inserting $\bot$ or recognizing complex zero changes is ever necessary to
...
Consulting our logical relation at set type, we know $e$ and $\phi e$ are
identical, and so the number of iterations never shrinks. However, as
demonstrated in \cref{sec:seminaive-examples}, if the body can be simplified to
$\bot$, rewriting $(\efor x e \bot)$ to $\bot$ can eliminate the loop entirely,
potentially an asymptotic improvement.
