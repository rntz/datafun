...
function $e$ computes, so you might expect us to use $\delta e$%
%.
, the derivative of $e$.
%
However, $e$ is not a function~\citep{magritte}: it is an expression computing a
function, and $\delta e$ is the derivative of this \emph{expression} --- it
computes how $e$ changes.
%
Recall from \cref{sec:derivatives} that a function's derivative is
\emph{a zero-change to that function}. So the change $\delta e$ and the
...
\todo{We discussed $\D(A \x B)$, $\D(A \to B)$, $\D\tseteq A$, and $\D\iso A$ in
  \cref{sec:change-structures}.} $\D\tunit = 1$ because there is only
