...
\usepackage{iftex}
\ifPDFTeX\PassOptionsToPackage{spacing=true,tracking=true,letterspace=30,}{microtype}\else\fi
...

  \begin{frame}
    ...
    \pause\vspace{2ex} Or, in Datalog:
    \begin{minipage}[t][.43\paperheight]{\paperwidth}
      \[\begin{array}{l}
      \n{reach}(\c{start}).\\
      \n{reach}(Y) \gets \n{reach}(X) \wedge \n{edge}(X,Y).\\[1ex]\pause
      \n{reach2}(\c{start}).\\
...

  \newcommand\chgat[4]{{\ensuremath{{#2} \dblcolon_{#1} {#3} \to {#4}}}}
  \newcommand\chg[3]{{\ensuremath{{#1} \dblcolon {#2} \to {#3}}}}

  \begin{frame}{Incremental \fn-calculus
      \textsuperscript{\sffamily[Cai et al \textsc{pldi} 2014, Giarrusso's thesis \textsc{2017},
  ...
  \newcommand\chgatcov[4]{{\uncover<2->{{#2} \dblcolon_{#1}} {#3} \to {#4}}}
...

  \begin{frame}\Large
    ...
    \vspace{1ex}
    \begin{itemize}
    \item \hi{$\phi e$} computes $e$ faster by replacing $\fix\;f$ by $\semifix\;(f,f')$.\\
      To do this, it needs to find $f'$, for which it uses...

    \item \hi{$\delta e$}, which computes how the expression $\phi e$ changes as
...

  \begin{frame}\centering\Huge\color{red}\scshape unfinished\end{frame}
