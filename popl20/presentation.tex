...
\usetikzlibrary{calc,tikzmark,arrows,positioning}  % for tikzmark
...

  \newcommand\lineto[3]{%
    \tikz[remember picture]{\draw[red,overlay,arrows=->] (#2) to[#3] (pic cs:#1)}}

  %% TODO: connect this up somehow, either arrows or hilighting in different
  %% colors.
  \begin{frame}
    \[\begin{array}{l}
    \n{reach} \isa \tpname{S\tikzmark{settype}et}~(\tnode \x \tnode) \to \tnode \to \tset \tnode\\
    \n{reach} \;\n{edge} \;\n{start} =
    \kw{fix}\; (\fnof R \{\n{start}\}
    \cup \tikzmark{comprehensions}\setfor{y}{x \in R, (x,y) \in \n{edge}})
    \end{array}\]
    \pause
    ...
    \item a finite set\lineto{settype}{0,0.3}{out=90,in=270} datatype
      \& set comprehensions, \pause
    \item a monotone fixed point operator, \pause
...

  ...
  \begin{frame}{Incremental \fn-calculus
      \textsuperscript{\sffamily[Cai et al \textsc{pldi} 2014, Giarrusso's thesis 2017,
