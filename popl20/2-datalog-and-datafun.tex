...
definitions like \name{ancestor} denote the set of fact-tuples deducible from
the rules defining them. More formally, we can view these rules as defining a
relation transformer, and taking its least fixed point. One of Datalog's key
syntactic restrictions is \emph{stratification}: a recursive definition cannot
refer to itself beneath a negation. For example, the liar paradox is prohibited:

\begin{code}
  \quad\name{liar}() \gets \neg \name{liar}().
  \qquad\color{Red}\textsf{\scshape\ding{55}\: not valid datalog}
\end{code}
