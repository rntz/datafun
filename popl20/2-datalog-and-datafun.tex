...
The \datalog{parent(_, _)} relation declares a set of
...
\datalog{arwen}, and so on. The ancestor relation is declared as
...
least fixed point of that function. Datalog's syntax is set up so the
...
So we need to allow non-monotone functions, while preserving the
...
\end{tabbing}
% This looks more like a definition one might see in an introductory mathematics course.
