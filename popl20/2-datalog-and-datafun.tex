...
$f\<e$), sums ($\inj i e$ and $\ecase{e} (\inj i x_i \caseto f_i)_{i\in\{1,2\}}$)
...
sets\footnote{To ensure implementability, set elements must be \emph{equality types}
  which support decidable equality. In our core calculus, we use a subgraamar of
...
equality tests $\dvar e = \dvar f$. For example, the test
$\eset{2, 3} = \eset{1, 2, 3}$ is false, but if the first argument is
increased to $\eset{1, 2, 3}$ then the test \emph{non-monotonically}
changes to true -- the boolean type $1 + 1$ is discretely ordered, so
a monotonic operation returning a boolean must always return the
same result if its argument is increased or decreased. 
...
matching eliminator $\eletbox x e f$. The variables are coloured
$\dvar x$ to indicate that they are discrete, and uncolored $x$ to
indicate they are monotonic, and discrete terms $\dvar e$ are restricted
to only refer to discrete variables. 
