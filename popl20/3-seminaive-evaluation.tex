...
\noindent
Observe that $\name{step} \<(\name{path} \cup \name{dpath}) =
\name{step}\<\name{path} \cup \name{step}'\<\name{dpath}$. \todo{give an
  equational proof of this!}
%
...
every type $A$ with a \emph{change structure}, consisting of:\footnote{Our
  ...
  \cref{sec:differences-from-incremental}. As we do not use change structures
  \emph{per se} in our proofs, we treat them informally, as a source of
