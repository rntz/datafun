\section{From Semi\naive{} Evaluation to the Incremental \texorpdfstring{\boldfn}{\textlambda}-Calculus}
...
Semi\naive\ evaluation avoids waste by transforming the rules for \name{path} to
find the \emph{newly} deducible paths, $\name{dpath}_i$, at iteration $i$, and
accumulating these changes to produce a final result:

\begin{flail}
  \name{dpath}_0(X,Y) &\gets \atom{edge}{X,Y}
  \\
  \name{dpath}_{i+1}(X,Z) &\gets \atom{edge}{X,Y} \wedge \name{dpath}_i(Y,Z)
  \\
