...
\noindent
Since only \emph{increasing} changes are necessary, and $\iso A$ is ordered
discretely, it permits no change at all:
...
%
Why not simply give changes pointwise, $\iso A \to \D B$? Consider
incrementalizing function application: we wish to know how $(f\<x)$ changes as
both $f$ and $x$ change. So fix $\changes{\df}{f}{g}$ and $\changes{\dx}{x}{y}$.
How do we find the change $f\<x \changesto g\<y$ that updates both function and
argument? If changes were given pointwise, taking only a base point, we'd
stipulate that $\changes{\df}{f} g$ iff $\fa{x} \changes{\df\<x}{f\<x}{g\<x}$.
But this only gets us to $g\<x$, not $g\<y$: we've accounted for the change in
the function, but not the argument.
%
The simplest solution is to give $\df$ an additional parameter: the change $\dx$
to $f$'s argument $x$. Then by inverting \textsc{FnChange} we have
$\changes{\df\<x\<\dx}{f\<x}{g\<y}$ as desired.
