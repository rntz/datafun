...
We borrow the explanation of semi\naive\ evaluation from
\citet{DBLP:conf/esop/Alvarez-Picallo19}. Consider the following Datalog
program:

%% TODO: not enough vertical white space around this. also, maybe syntax highlighting?
\begin{verbatim}
    path(X,Y) :- edge(X,Y).
...
\noindent
The denotation of \texttt{path} is the least fixed point of the rules defining
it.
...
%
For example, if the \texttt{edge} relation is \{(1,\,2), (2,\,3), (3,\,4)\}, we
...
\emph{semi\naive\ evaluation}~\cite{seminaive}. \todo{finish}
